...
# bias表示分块操作时产生的偏置
parser = argparse.ArgumentParser(description='find endpoint neighbor with kimimaro')1
parser.add_argument('--label', '-l', type=str, default='resultsforyuanjb0427/', help='label file path')######### data path
# parser.add_argument('--label', '-l', type=str, default='elastic/', help='label file path')
# parser.add_argument('--label', '-l', type=str, default='SCN_small/', help='label file path')
...
parser.add_argument('--dust', '-du', type=int, default=12000, help='kimimaro skeleton dust threshold')######### big for less, small for more
        ...
        for y_part in range(y_num):
            for z_part in range(z_num):
                ...
                if z_part < z_num-1:
                    ...
                    for seg_loc in seg_list[z_first * z_part:]:
                        im = io.imread(args.label + seg_loc).astype(np.int32)
                        im = im.T
                        if labels_original is None:
                            labels_original = im[int(x_part*(x_shape_original/x_num)): int((x_part+1)*(x_shape_original/x_num)), int(y_part*(y_shape_original/y_num)): int((y_part+1)*(y_shape_original/y_num))]
