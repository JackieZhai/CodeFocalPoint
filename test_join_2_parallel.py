...
# ks_lock: the shared lock of 'kimimaro_stack' function
def kimimaro_stack(stack_name, big_skels, ks_lock):
    print('--', stack_name)
...
big_skels = Manager().dict()
ks_lock = Manager().Lock()
ks_partial = partial(kimimaro_stack, big_skels=big_skels, ks_lock=ks_lock)
with Pool(processes=plnm) as pool:
    pool.map(ks_partial, stack_name_list)
